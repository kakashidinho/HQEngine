...
namespace MyGUI
{
	...
	public:
		...
		virtual void textureInvalidate(ITexture* _texture);//implementing method of ITextureInvalidateListener
	...
	private:
		...
		template<bool LAMode, bool Antialias>
		void renderGlyphs(const GlyphHeightMap& _glyphHeightMap, const FT_Library& _ftLibrary, const FT_Face& _face, ITexturePixelBuffer* _texBuffer, int _texWidth, int _texHeight);
		...
		void renderGlyph(GlyphInfo& _info, uint8 _luminance0, uint8 _luminance1, uint8 _alpha, int _lineHeight, ITexturePixelBuffer* _texBuffer, int _texWidth, int _texHeight, int& _texX, int& _texY, uint8* _glyphBuffer = nullptr);
