...
HQReturnVal HQRenderDeviceDebugLayer:: BeginRender(HQBool clearPixel,HQBool clearDepth,HQBool clearStencil,HQBool clearWholeRT)
{
	HQReturnVal re = m_pDevice->BeginRender(clearPixel , clearDepth , clearStencil , clearWholeRT);
	
	if (HQFailed(re))
	{
		sprintf(debugString , 2048 , "HQRenderDevice::BeginRender(%d , %d , %d , %d) return %s\n" ,
			(int)clearPixel , (int)clearDepth , (int)clearStencil ,(int) clearWholeRT , HQReturnValToString(re));
...
HQReturnVal HQRenderDeviceDebugLayer::Clear(HQBool clearPixel,HQBool clearDepth,HQBool clearStencil,HQBool clearWholeRT)
{
	HQReturnVal re = m_pDevice->Clear(clearPixel , clearDepth , clearStencil , clearWholeRT);
	if (HQFailed(re))
	{
...
#endif//#if !defined GLES && !(defined HQ_WIN_PHONE_PLATFORM || defined HQ_WIN_STORE_PLATFORM)
