...
HRESULT AllocMeshHierarchy::CreateFrame(LPCSTR Name, D3DXFRAME** ppNewFrame)
{
...
HRESULT AllocMeshHierarchy::CreateMeshContainer(LPCSTR Name, 
...
HRESULT AllocMeshHierarchy::DestroyMeshContainer(D3DXMESHCONTAINER* pMeshContainerBase)
{
	...
	for(DWORD i = 0; i < pMeshContainerBase->NumMaterials; ++i)
	{
		...
		//pMeshContainerBase->pMaterials[i].MatD3D=0;
	}

	delete[] pMeshContainerBase->pMaterials;
	
	SafeRelease(pMeshContainerBase->MeshData.pMesh);
	SafeRelease(pMeshContainerBase->pSkinInfo);
